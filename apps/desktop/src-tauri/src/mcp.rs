//! Callimachus MCP server — exposes the local thread index as tools any LLM/agent
//! can call over stdio. Reuses the same search + context core as the desktop app
//! against the same `index.db` (read-only intent; opened read-write only so the
//! embedding model can be built on demand for semantic queries).
//!
...
use callimachus_lib::{context, db, embed, search};
